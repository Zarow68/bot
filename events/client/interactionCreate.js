module.exports = {
    ...
    async execute(client, interaction) {
        ...
        if (!guildSettings) {
            ...
            return interaction.reply('Le bot a mis à jour la base de données pour votre serveur, retaper la commande !');
        }
        if (interaction.isCommand() || interaction.isContextMenu()) {
            ...
            if (cmd.ownerOnly) {
                if (interaction.user.id != ownerId) return interaction.reply('Les seule personnes pouvant taper cette commande est l\'owner du bot !');
            }

            if (!interaction.member.permissions.has([cmd.permissions])) return interaction.reply({ content: `Vous n'avez pas la/les permissions(s) requise(s)(\`${cmd.permissions.join(', ')}\`) pour taper cette commande ! `, ephemeral: true});
